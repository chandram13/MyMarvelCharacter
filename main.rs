...
use std::env::{args, Marvel}

fn firstFunction() {
    let mut myBrand: Args = args();

    let character_name = character_name.parse::<String>().unwrap();
    let character_power = character_power.parse::<String>().unwrap()
    let character_weakness = character_weakness.parse::<String>().unwrap()
    let result = operate(character_name, character_power, character_weakness);
